...
class RoomService:
    def __init__(self, room_repo: RoomRepository, invite_repo: InviteRepository) -> None:
    ...
    async def create(self, privacy_mode: PrivacyMode, ttl_seconds: int) -> CreatedRoom:
        ...
        saved = await self._room_repo.create(entity)

        await self._invite_repo.create(InviteEntity(
            room_id=saved.id,
    ...
    async def get_by_code(self, room_code: str) -> RoomEntity | None:
        return await self._room_repo.get_by_code_hash(sha256(room_code))
